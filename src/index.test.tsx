...
describe('reducer branch coverage', () => {
  ...
  describe('Async dispatch functionality', () => {
    it('should handle async dispatch with direct promise', async () => {
      const { useReduxLiteStore, ReduxLiteProvider } = initiate(INIT_STORE);
      ...
      const TestComponent = () => {
        ...
        const handleAsyncUpdate = async () => {
          const userPromise = Promise.resolve({ name: 'AsyncUser', age: 25, data: { name: 'async-data', value: 333 } });
          await store.dispatchAsyncUser(userPromise);
        };
      ...
      act(() => {
        getByText('Update Async').click();
      });
      
      // Wait for the async operation to complete
      await waitFor(() => {
        expect(getByText('User Name: AsyncUser')).toBeDefined();
        expect(getByText('User Age: 25')).toBeDefined();
        expect(getByText('User Data: {"name":"async-data","value":333}')).toBeDefined();
      });
    });

    it('should handle async dispatch with function callback', async () => {
      const { useReduxLiteStore, ReduxLiteProvider } = initiate(INIT_STORE);
      ...
      const TestComponent = () => {
        ...
        const handleAsyncUpdate = async () => {
          await store.dispatchAsyncUser(async (getPrevUser, getFullState) => {
            ...
            return {
              ...prevUser,
              name: prevUser.name + '_updated',
              age: fullState.count + 10
            };
          });
        };
        ...
        return (
          <div>
            <button onClick={handleAsyncUpdate}>Update Async Function</button>
            <div>User Name: {store.user.name}</div>
            <div>User Age: {store.user.age}</div>
          </div>
        );
      ...
      // Trigger async update
      act(() => {
        getByText('Update Async Function').click();
      });
      
      // Wait for the async operation to complete
      await waitFor(() => {
        expect(getByText('User Name: Jhon_updated')).toBeDefined();
        expect(getByText('User Age: 16')).toBeDefined(); // 6 (count) + 10
      });
    });

    it('should handle async partial dispatch', async () => {
      const { useReduxLiteStore, ReduxLiteProvider } = initiate(INIT_STORE);
      
      const TestComponent = () => {
        const store = useReduxLiteStore();
        
        const handleAsyncPartialUpdate = async () => {
          const partialPromise = Promise.resolve({ age: 35 });
          await store.dispatchAsyncPartialUser(partialPromise);
        };
        
        return (
          <div>
            <button onClick={handleAsyncPartialUpdate}>Update Async Partial</button>
            <div>User Name: {store.user.name}</div>
            <div>User Age: {store.user.age}</div>
            <div>User Data: {JSON.stringify(store.user.data)}</div>
          </div>
      ...
      // Trigger async update
      act(() => {
        getByText('Update Async Partial').click();
      });
      
      // Wait for the async operation to complete
      await waitFor(() => {
        expect(getByText('User Name: Jhon')).toBeDefined(); // Should remain unchanged
        expect(getByText('User Age: 35')).toBeDefined(); // Should be updated
        expect(getByText('User Data: {"name":"data-test","value":111}')).toBeDefined(); // Should remain unchanged
      });
    });

    it('should handle async partial dispatch with function callback', async () => {
      const { useReduxLiteStore, ReduxLiteProvider } = initiate(INIT_STORE);
      ...
      const TestComponent = () => {
        ...
        const handleAsyncPartialUpdate = async () => {
          await store.dispatchAsyncPartialUser(async (getPrevUser, getFullState) => {
            // Simulate async operation
            await new Promise(resolve => setTimeout(resolve, 10));
            return {
              age: getPrevUser().age + getFullState().count
            };
          });
        };
        ...
        return (
          <div>
            <button onClick={handleAsyncPartialUpdate}>Update Async Partial Function</button>
            <div>User Name: {store.user.name}</div>
            <div>User Age: {store.user.age}</div>
          </div>
        );
      ...
      // Trigger async update
      act(() => {
        getByText('Update Async Partial Function').click();
      });
      
      // Wait for the async operation to complete
      await waitFor(() => {
        expect(getByText('User Name: Jhon')).toBeDefined(); // Should remain unchanged
        expect(getByText('User Age: 36')).toBeDefined(); // 30 (prev age) + 6 (count)
      });
    });
    ...
    it('should propagate errors from async operations', async () => {
      ...
      const TestComponent = () => {
        ...
        const handleAsyncUpdate = async () => {
          try {
            ...
            await store.dispatchAsyncUser(failingPromise);
          } catch (err) {
        ...
        return (
          <div>
            <button onClick={handleAsyncUpdate}>Update Async Error</button>
      ...
      // Initial state
      expect(getByText('Error: None')).toBeDefined();
      
      // Trigger async update
      act(() => {
        getByText('Update Async Error').click();
      });
      
      // Wait for the async operation to complete
      await waitFor(() => {
        expect(getByText('Error: Async operation failed')).toBeDefined();
      });
    });

    it('should propagate errors from async partial operations', async () => {
      const { useReduxLiteStore, ReduxLiteProvider } = initiate(INIT_STORE);
      ...
      const TestComponent = () => {
        ...
        const handleAsyncUpdate = async () => {
          try {
            const failingPromise = Promise.reject(new Error('Async partial operation failed'));
            await store.dispatchAsyncPartialUser(failingPromise);
          } catch (err) {
            setError((err as Error).message);
          }
        };
        ...
        return (
          <div>
            <button onClick={handleAsyncUpdate}>Update Async Partial Error</button>
            <div>Error: {error || 'None'}</div>
          </div>
        );
      };
      ...
      // Trigger async update
      act(() => {
        getByText('Update Async Partial Error').click();
      });
      
      // Wait for the async operation to complete
      await waitFor(() => {
        expect(getByText('Error: Async partial operation failed')).toBeDefined();
      });
    });
  });